...
/// 每行待办后面的那行时间。
///
/// 为什么不用 `Date.FormatStyle`：它跟着系统区域设置走，同一条待办在别人
/// 机器上会长成 `8/30/26` 或 `30.08.2026`，而这个界面其余每一个字都是写死的中文
/// —— 混在一起更难读，也写不了用例。这里自己拼，拼出来的东西在哪台机器上都一样。
///
/// 「今天 / 昨天」不是装饰：待办绝大多数是今天写的，一列如果全是
/// `8月30日`，日期这一栏就等于没有信息 —— 眼睛真正要区分的是「刚写的」和「拖了很久的」。
enum TodoDate {
