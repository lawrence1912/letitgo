...
不是一个默认的 AppKit 窗口：一颗能量星球铺在主窗口底部，
窗口 chrome、控件和卡片是压在它上面的**系统 Liquid Glass**（macOS 26）。

- **六套色板**（棱镜 / 星云 / 霜白 / 莫兰迪 / 雾青 / 素白，默认棱镜），设置里随时切；每个色写成 OKLCH 的
  四组值（浅 / 深 / 增强对比度浅 / 深），绘制时按当前外观挑。换主题换的是**整个界面** ——
  连边栏的选中块、滑块、勾选框、聚焦环这些系统控件一起换。
- **默认那套是「棱镜」**：最主流的那套**科幻蓝** —— 近黑深蓝打底，主操作是电光青，
  青 / 电蓝 / 靛紫组成的全窗口能量星球不占页面布局空间。
  「星云」是暖调深空 + 琥珀（还多一层 40pt 的细网格）；想安静就换霜白 / 素白。
- **星球是个光源，不是一张贴图**：整颗球服从一个方向 —— 光从左上打过来，
  能量环在那一侧烧成一道近白的热弧，绕回右下衰减成微光；球体本身是暗的，
  左上一道贴着球面的边缘光交代它有体积。
- **它也不只是好看**：`glassEffect` 折射的是**背后真实存在的内容**，
  背后什么都没有的玻璃只会渲染成一块灰板。所以那颗星球和那层细网格是这套材质的
  **前提** —— 它们提供了可折射的起伏和可对齐的规律。
- **四档材质，不是「什么都做成玻璃」**：近黑的窗口场 → 氛围（星球 + 细网格）
  → **烟熏面**（大卡：系统材质 + 压沉的幕 + 细边 + 投影，**不上玻璃**）
  → **交互玻璃**（选中的导航项、分段控件、按钮）。
  分界线是：玻璃是给控件和导航的；一张大卡的活儿是沉下去、把注意力让给里面的字。
- **补的两层光浓度差一个数量级**：斜光（白 0.07，压在正文底下，进对比度合成链）
  和镜面边（1px，可以亮到 0.85，正文压不到它）。
  这两个数是从「一排塑料糖豆」那一版收回来的 —— 光该是察觉不到的一层。
- **阅读区使用稳定底色**：输入框、代码和结果区隔开背景纹理，清透玻璃留给导航和短控件。
- **换主题保留当前编辑状态**：颜色更新会触发重绘，输入、选项和撤销记录不会随主题清空。
- **按钮是自己画的，但只画**：`ButtonStyle` 只接管画法（点击 / ⏎ / Tab 焦点 /
  禁用传播一样不丢），底下那层材质仍然是系统的 `glassEffect`。
  在它上面补三样系统不给的东西 —— 顶面那道亮带、一圈有方向的镜面边、
  主操作底下漏出的辉光。
- **一屏只有一个响亮的东西**：只有主操作发光；「当前选中」低一档；别的不上色。
  破坏性操作**不发光** —— 辉光是在邀请人按它，它要好认不该好按。
- **对比度是算过的，但这一版算得比上一版少**：系统材质怎么混不该由我们假设，
  所以测试退回到还归自己画的两块底 —— 氛围场（下界）和读写面（精确）。
  正文 ≥ 4.5:1，增强对比度那两组 ≥ 7:1，有测试兜着。
- **无障碍四项全要**：减弱动态、减弱透明度、增强对比度、完整键盘 + VoiceOver。
  这一版大部分交还给了系统控件 —— 还给系统的那些，白拿上下键遍历、
  首字母跳转、`.isSelected` 语义和减弱透明度的自动让位。

完整规范见 [`DESIGN.md`](DESIGN.md)，产品原则见 [`PRODUCT.md`](PRODUCT.md)。
DESIGN.md 文末记了八个踩过的 SwiftUI 坑，前三条都是「编译通过、看不出错、
...
要求 macOS 26+ 和 Swift 6 工具链（Liquid Glass 是 26 才有的）。**只装 Command Line Tools 也能跑**，
不需要完整 Xcode（代价见文末）。

产物在 `build/LetItGo.app`，可以直接双击。待办的数据落在
